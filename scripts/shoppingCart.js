...
$(function () {
  ...
  /*  //选框变色
    ...
    //计算小计
    function totalRow(obj,num) {
        var total = (parseFloat($(obj).parent().prev().find("span").text())*num);
        if(!Boolean(total) || total<0){
            $(obj).parent().next().find("span").text("0");
        }else{
            $(obj).parent().next().find("span").text(total.toFixed(2));
        }
    };
    ...
    function statistics() {
        ...
        $(".carConDetail").each(function () {
            if($(this).find(".checkRow").prop("checked")){
                selectedNum += parseInt($(this).find(".quantity .num").val());
                totalPrice += parseFloat($(this).find(".totalRow span").text());
            }
            ++ kindQuantity;
        });
